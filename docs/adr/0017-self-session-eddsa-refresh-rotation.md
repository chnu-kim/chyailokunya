...
2. **refresh = opaque 난수(32B) + sha256 해시만 DB 저장 · httpOnly 쿠키.** 14일 sliding +
   **90일 absolute cap**(family 첫 로그인 기준).
   - **예외 하나를 명시한다**: 아래 3의 grace 멱등 반환을 위해 `replaced_by_token` 에 **후계
     토큰의 평문**이 잠시 들어간다. 이건 "평문을 저장하지 않는다"의 유일한 구멍이므로
     **grace 를 넘긴 값은 반드시 지워야 한다** — `cleanupRefreshTokens` 를 로그인·회전 성공
     경로에서 곁다리로 호출해 창을 닫는다. 이 청소가 돌지 않으면 _현재 살아 있는_ 토큰의
     평문이 DB 에 영구히 남아, DB 를 한 번 읽는 것만으로 활성 세션을 탈취할 수 있다(실제로
     초판에서 호출자가 빠져 있었고 적대적 리뷰가 잡았다).
3. **회전 + 재사용 감지.** 조건부 UPDATE claim(D1 단일 writer 라 원자적)으로 **최초 회전자만** 승계한다.
   이미 회전된 토큰이 오면:
   - **grace 30초 내** → 정상 동시 탭. **저장된 후계를 멱등 반환**한다(새로 발급하지 **않는다** —
     발급하면 도둑이 유효 토큰을 무한정 찍어내 도난 감지가 무력화된다. adversarial 이 잡은 함정).
...
5. **자동 갱신 = `src/proxy.ts`**(Next16 의 middleware rename). OpenNext 라 엣지가 아니라 **Node
   런타임**이어서 D1·jose 가 그대로 된다. access 가 유효하면 검증만(DB 0), 만료면 회전하고 갱신된
   쿠키를 downstream 으로 forward 한다.

부수 결정: **CSRF 2겹**(SameSite=Lax + **Origin 헤더 검증**) · **state 는 쿠키 이중제출**(DB 미사용,
httpOnly·10분) · **로그아웃 = 현재 기기의 family 폐기**(다중 기기는 family 로 자연 분리) · 키는
시크릿 `JWT_SIGNING_JWK`(private) / `JWT_PUBLIC_JWK`(public), `kid="v1"`.
