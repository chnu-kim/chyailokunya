...
1. **access = EdDSA(Ed25519) JWT · 15분 · httpOnly 쿠키 · 신원만.** jose 로 서명/검증하고
   ...
   뿐이다. 비대칭이라 검증 측(middleware·route·서버 컴포넌트)은 public key 만 갖는다.
...
> **로그아웃은 쿠키 삭제만으로 확정되지 않는다.** 로그아웃 직전에 middleware 가 회전 중이던 요청의
...
> 마커 쿠키(`ck_lo`)를 남기고, 이후 요청에서 middleware·세션 읽기가 세션 쿠키를 무시·삭제한다.
