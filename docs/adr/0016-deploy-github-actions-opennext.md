...
- **트리거**: `workflow_run` — CI("게이트")가 **main 에서 push 로 성공**한 뒤에만. 실패·취소·
  pull_request 트리거·다른 저장소면 안 함(미머지/fork 코드가 프로덕션 자격으로 배포되는 구멍 차단).
- **순서**: `opennextjs-cloudflare build`(산출물 검증) → `wrangler d1 migrations apply --remote`(멱등)
  → `opennextjs-cloudflare deploy`(사전 빌드 업로드, 재빌드 없음). 빌드를 마이그레이션보다 **먼저** 해
  빌드 실패가 D1 을 안 건드리게 하고, 롤아웃 창의 500 을 구조적으로 없앤다. 마이그레이션은
  expand-contract(하위호환)로 둬 그 뒤 업로드가 실패해도 구 워커가 새 스키마에서 안전하게 한다.
- **stale-run 가드**: `head_sha` 가 현재 `origin/main` tip 일 때만 배포(오래된 CI 성공 run 을
  재실행해 옛 커밋을 롤백 배포하는 것 차단).
