...
/* 마이그레이션 0012 — 주 메타에 팬아트 두 칸을 더한다(ADR-0028).

   **0008 과 정확히 같은 자리다.** drizzle-kit 이 CHECK 추가 때문에 테이블을 재생성하면서
   `INSERT … SELECT "fanart_image_key", "fanart_credit" … FROM schedule_weeks` 를 냈는데, 그
...
const COLUMNS = "week_start_date, note, draft, published_at, created_at, last_updated_at" as const;
...
test("0012 이관: 기존 주가 그대로 살고 팬아트 두 칸이 NULL 로 붙는다", () => {
  ...
  expect(
    db
      .prepare(
        "SELECT count(*) AS n FROM schedule_weeks WHERE fanart_image_key IS NOT NULL OR fanart_credit IS NOT NULL",
      )
      .get(),
  ).toEqual({ n: 0 });
...
test("0012: 작가 표기만 있고 그림이 없는 조합을 DB 가 거절한다", () => {
  ...
  db.exec(
    `INSERT INTO schedule_weeks (week_start_date, fanart_image_key, created_at, last_updated_at)
     VALUES ('2026-09-14', '0189d1f0-3a4b-7c8d-9e0f-1a2b3c4d5e6f.png', 1784000005000, 1784000005000);`,
  );
  expect(
    db
