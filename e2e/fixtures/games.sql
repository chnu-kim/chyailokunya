...
VALUES
  ...
  -- 셋째 주는 **공지를 지우는 스펙 전용**이다(2026-08-01, 적대적 리뷰 2라운드). 위 둘은 긴 공지가
  -- 카드에 어떻게 앉는지를 읽기만 하는데, "비우고 저장하면 내려간다"는 쓰기라 같은 주를 쓰면
  -- 그 스펙이 위 둘의 조건을 지워 버린다(D1 픽스처를 모든 스펙이 공유한다 — AGENTS). 길이는
  -- 짧아도 된다: 여기서 재는 것은 clamp 가 아니라 "칸이 뜨고, 비우면 값이 내려가는가"다.
  ('2030-09-02', '내려갈 옛 공지', 0, 1700000020000, 1700000020000, 1700000020000);
...
VALUES
  ...
  ('2030-09-02', '옛 공지 주의 항목', NULL, 1700000020000, 1700000020000);
