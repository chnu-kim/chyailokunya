  ...
  return {
    test: {
      ...
      coverage: {
        ...
        thresholds: {
          /* 결정 28(하루 칸 아코디언)과 결정 30(휴방 빈 줄 정리)이 같은 주에 나란히 머지되며
             바닥이 두 번 올랐고, 결정 35(카드 조작 재구성)가 한 번 더 올렸다. 결정 32·33(머리
             줄 · 항목 한 상자)이 네 번째다 — 실측 81.29/77.03/82.97/82.78(2026-08-01).
             아래 값은 목표가 아니라 **이 시점 바로 아래에 박은 래칫**이다.

             **결정 35 는 코드를 지우면서도 비율이 올랐다** — `!card` 이른
             반환 가지를 통째로 걷어 분모가 줄었는데(아래 functions 주석의 "코드를 지우면
             내려갈 수 있다"와 반대 방향), 같은 PR 이 variant·busy·접근명 계약을 새로 덮어
             분자가 더 크게 늘었다. 방향은 그때그때 다르니 짐작하지 말고 매번 실측해서 박는다.
             결정 32·33 은 JSX 구조만 옮겨 분모가 거의 안 바뀌었고 새 dom 테스트 둘이 분자를
             밀어 statements·lines 는 소수 둘째 자리에서만 움직였다.

             **결정 36(팬아트 슬롯)이 다섯 번째다 — 실측 81.61/77.35/83.33/83.19(2026-08-01).**
             드래그 배선이 들어오며 분모가 20줄쯤 늘어 바닥을 한 번 **밑돌았고**(82.28 <
             82.7), 래칫을 내리는 대신 그 배선을 덮는 dom 스펙 다섯을 더해 도로 올렸다 —
             AGENTS 의 "테스트를 늘리는 PR 은 임계치도 같이 올린다"가 반대 방향으로 작동한
             자리다(새 코드가 바닥을 깎으면 테스트로 되받는다). */
          statements: 81.6,
          branches: 77.3,
