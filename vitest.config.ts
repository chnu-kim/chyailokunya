      ...
      coverage: {
        ...
        thresholds: {
          /* 결정 28(하루 칸 아코디언)과 결정 30(휴방 빈 줄 정리)이 같은 주에 나란히 머지되며
             바닥이 두 번 올랐고, 결정 35(카드 조작 재구성)가 한 번 더 올렸다 — 실측
             81.25/76.98/82.81/82.72(2026-08-01). 아래 값은 목표가 아니라 **이 시점 바로 아래에
             박은 래칫**이다.

             **결정 35 는 코드를 지우면서도 비율이 올랐다** — `!card` 이른 반환 가지를 통째로
             걷어 분모가 줄었는데(아래 functions 주석의 "코드를 지우면 내려갈 수 있다"와 반대
             방향), 같은 PR 이 variant·busy·접근명 계약을 새로 덮어 분자가 더 크게 늘었다.
             방향은 그때그때 다르니 짐작하지 말고 매번 실측해서 박는다. */
          statements: 81.2,
          branches: 76.9,
