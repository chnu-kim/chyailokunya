...
unit+커버리지 · drizzle-kit check · build · **배포 빌드(opennextjs-cloudflare)** ·
**배포 번들 스모크(workerd)**` 게이트와 **e2e 스모크**(별도 job, axe 접근성 스캔 포함)를 돌린다.
배포 빌드가 게이트에 있는 이유는 아래 Phase 4 지뢰를 보라 — `next build` 만으론 배포 실패를 못
잡는다. **그리고 빌드가 성립하는 것과 그 번들이 부팅해 페이지를 서빙하는 것은 다른 사실이라**,
방금 만든 `.open-next` 를 `wrangler dev` 로 띄워 스모크를 한 번 돌린다(층 4). 배포 후엔
`deploy.yml` 이 **같은 스크립트**를 프로덕션 origin 에 대고 돌린다(층 5) — 판정이 배포 전후로
갈리지 않게 스크립트를 하나만 둔다. **시각 스냅샷은 CI 에 없다** —
...
- **주간표 PNG 서버 렌더(og:image 겸용, 작업순서 7)를 프로덕션 인시던트로 되돌렸다 — 결정 15
  잠정 철회.** Satori/resvg(`next/og`)의 고정 렌더 비용이 Cloudflare Workers 무료 플랜의
  요청당 CPU 10ms 한도를 구조적으로 못 지킨다. 발행된 주의 og:image URL 을 소셜 크롤러가
  자동으로 두드릴 때마다 그 isolate 가 CPU 한도를 넘겨 죽고, 같은 isolate 를 타던 무관한
  요청(favicon.ico 까지)도 연쇄로 실패했다(Error 1102, 2026-07-27 실측 `wrangler tail`).
  **로컬 게이트 8종 중 어느 하나도 이걸 못 잡는다** — `next build`·`opennextjs-cloudflare
build`·`npm run preview`(로컬 workerd) 전부 실제 프로덕션 CPU 시간을 강제하지 않는다
  (Phase 4 지뢰의 "배포 계약"과 같은 급이지만 한 단계 더 깊다 — 빌드는 성립해도 런타임
  정책이 다르다). `/api/og/schedule` 라우트를 통째로 지웠다 — 자동 og:image 링크는 당분간
  사이트 기본 커버(`OG_IMAGE`)로 고정하고, PNG 는 og:image 자동 링크가 아니라 **클라이언트
  사이드**(방문자 브라우저) 렌더 + 수동 다운로드로 다시 설계한다(별도 이슈). 그 라우트가
  살아있던 동안 잡았던 두 교훈은 재구현 시에도 유효하다: (1) Satori 가 CSS 로 끼워 넣는 생성
  콘텐츠(`text-overflow: ellipsis` 등)는 렌더 문자열을 아무리 훑어도 폰트 서브셋 수집에 안
  잡힌다 — 실데이터에서 "…"(U+2026) 가 두부(□)로 나온 뒤에야 발견됐다(게이트 8종·e2e 는
  상태 코드·content-type 만 봐서 전부 초록이었다). (2) `Cache-Control` 헤더만으로는
  Cloudflare 가 Worker 응답을 엣지에 캐싱해 주지 않는다 — Worker 가 zone 캐시보다 먼저
  요청을 받아, 직접 `caches.default`(Cache API)를 안 쓰면 헤더는 브라우저·소셜 플랫폼 쪽
  캐시에만 통한다.
...
- **히스토리 엔트리를 쓰는 모달을 닫은 **직후** `page.goto` 하면 ERR_ABORTED 다.** 닫힘이
  ...
  모달이 실제로 사라졌는지(`toHaveCount(0)`)를 먼저 기다린 뒤 이동한다. **모달이 겹쳐 있으면
  그 대기로도 부족하다** — 위가 닫히며 아래까지 닫는 배선이 있으면 back 이 한 번 더 날아가고,
  DOM 에서 사라진 뒤에도 왕복이 남는다. 그때는 **신원을 바꿔 같은 페이지를 다시 여는 대신
  `browser.newContext()` 로 다른 컨텍스트를 연다**(suggestions.spec 의 openAsAdmin) — 히스토리와
  아예 무관해지고, 관리자가 다른 사람이라는 사실과도 맞는다.
...
팬아트 관리자 UI·치수(ADR-0030)에서 밟은 것들:
...
- **머신이 특정 상태에서만 받는 이벤트는 화면이 그 상태를 잠가야 한다.** 팬아트 조작을 `ready`
  전용으로 두고 화면이 `uploading` 만 잠갔더니, **저장 중** 파일 선택·내리기가 아무 문구 없이
  드롭됐다. 파일 input 은 더 나쁘다 — 같은 파일 재선택을 살리려고 `value` 를 먼저 비우므로,
  드롭된 뒤엔 그 파일을 다시 골라도 `change` 가 안 난다(버튼이 죽은 것으로 보인다). 가드는
  방어선이고 잠금은 표시다 — 둘의 조건이 갈리면 조용한 무시가 된다.
- **`<label>` 에는 `:disabled` 가 안 붙는다.** 파일 input 을 label 로 감싸 버튼처럼 쓰면 안의
  input 이 잠겨도 `.btn:disabled` 가 안 걸려, 나란한 버튼만 흐려지고 이쪽은 멀쩡해 보인다.
  `:has(input:disabled)` 로 같은 처리를 준다(포커스 링도 같은 이유로 `:focus-within` 이다 —
  `.sr-only` 인 input 은 1px 상자라 제 자리에 링을 못 그린다).
- **`undefined = 유지` 규약을 쌍으로 움직이는 값에 그대로 적용하면 조용한 무시가 된다.** 폭만
  보낸 요청을 서버는 무시할 수밖에 없는데(쌍이 깨진 값은 DB CHECK 를 통과 못 한다) 그러면
...
PNG 공유 카드(#122)에서 밟은 것들 — **셋 다 게이트가 전부 초록인 채로 살아 있었다**:

- **`html-to-image` 는 computed style 을 SVG `foreignObject` 안으로 그대로 베낀다 — 캡처 대상
  노드에 화면 밖 배치를 걸면 받아지는 PNG 가 통째로 빈다.** 복제본에 `position: fixed;
top/left: -10000px` 을 걸었더니 그 값이 SVG 로 따라 들어가(루트 style 의
  `inset: -10000px …`) 카드가 1200×630 화폭 밖에 놓였다 — 전 픽셀 알파 0. 화면 밖 배치는
  **감싸는 상자**에 건다(그 상자는 캡처 대상이 아니라 SVG 로 안 샌다).
- **같은 세션의 두 번째 캡처만 그림을 빠뜨린다.** SVG 엔 그때도 data URL 이 제대로 들어간다 —
  지는 것은 래스터화다: 첫 캡처는 네트워크 왕복 동안 브라우저가 그 data URL 을 디코드해 두는데,
  두 번째는 라이브러리 캐시에 맞아 요청 없이 곧바로 그려 안쪽 그림이 디코드 전이다(6번 연속
  캡처 중 2번만 빔 — 실측). **캡처 전에 우리가 인라인하고 `img.decode()` 까지 끝낸다.**
- **산출물 게이트는 파일이 "유효한가"가 아니라 "무엇이 담겼나"를 봐야 한다.** 위 둘 다 PNG
  매직 바이트와 2400×1260 단언을 그대로 통과했다(빈 그림도 유효한 PNG 이고 치수도 맞다).
  이제 e2e 가 단색 픽스처를 올려 **받은 파일의 픽셀**을 찍는다 — 그림 자리의 색과, 같은
  프레임에서 목록 자리가 그 색이 **아닌지**를 함께 본다.
- **저장 상한이 100자인 문자열은 화면에서도 100자까지 서 본다.** 사진지 표기에 줄 수 제한이
  없어 15자에서 이미 상자를 넘었고, 공백 없는 라틴 핸들은 **가로로** 샜다. 길이 상한이 있는
  입력을 화면에 그릴 땐 그 상한값으로 한 번 렌더해 본다(e2e 가 그 값으로 잰다).

프로덕션 시계(2026-07-31 인시던트)에서 밟은 것들:

- **`Temporal.Now` 는 배포된 Workers 에서 에포크 0 을 돌려준다 — 시계는 `Date.now()` 로만 읽는다.**
  `?week=` 없는 `/schedule` 이 **1969-12-29 주**를 그렸고 "오늘" 칩은 어느 주에서도 안 떴다(주를
  명시한 요청 포함). 원인은 `temporal-polyfill` 의 진입점이 `NativeTemporal || 폴리필` 인데
  **프로덕션 workerd 엔 네이티브 Temporal 이 있고 그 `Now` 가 요청 시계에 안 물려 있다**는 것이다.
  같은 런타임에서 `Date.now()` 는 정확했고(쿠키 `Expires` 로 확인) Temporal 의 **산술·존 변환도
  정확**했다 — 깨진 건 `Now` 하나다. eslint `no-restricted-syntax` 가 이제 그 API 를 금지한다.
- **런타임이 API 를 다르게 구현하는 부류는 로컬 게이트가 구조적으로 못 본다.** 유닛(vitest
  workerd)·로컬 preview(wrangler dev) 둘 다 네이티브 Temporal 이 **없어** 폴리필이 맞는 답을 주고,
  e2e 는 `next dev`(Node)라 아예 다른 런타임이다 — 셋 다 초록인 채 라이브만 1970 이었다. 이런
  자리는 층 5(배포 후 스모크)가 유일한 관측점이고, **고침과 그 관측을 같은 PR 에 넣는다**(로컬
  초록이 아무것도 증명하지 못하므로 배포가 곧 검증이다).
- **배포 후 스모크의 "버전에 민감한 단언"은 전파를 기다린 뒤에 잰다.** 스모크는 `wrangler deploy`
  직후 0.25초에 시작하는데 그 시점엔 구 버전을 서빙하는 콜로가 있다(실측: +2.2초 구 버전 ·
  +4.5초 새 버전). 상태 코드·content-type·og:url·번들은 구 버전이 답해도 통과해서 여태 안
  드러났을 뿐이고, "이 배포가 시계를 어떻게 읽는가" 같은 단언을 넣는 순간 표면화된다.
  **스크립트에 재시도를 넣지 않는다** — "다시 해 보니 되더라"를 staleness 로 단정하면 진짜 간헐
  실패를 삼키고, 재시도 이전 본문이 번들 검사로 새어 나간다(리뷰 두 채널이 각각 짚었다).
  `deploy.yml` 이 15초 기다리고 스크립트는 첫 응답을 그대로 믿는다 — 억제가 아니라 순서로 푼다.

편집기 UI 개편(#56 결정 24~27, PR #130·#131·#132)에서 밟은 것들 — **여기 적힌 것 대부분이
게이트 8종을 전부 초록으로 통과한 채 리뷰에서만 잡혔다**:

- **"화면과 저장이 같다"를 주장하려면 둘이 같은 정규화 함수를 통과해야 한다.** 미리보기 카드를
  `draft` 로 그리면서 저장 페이로드(`draftEntryInputs`·`draftDayInputs`)와 다른 값을 넘겼더니,
  **빈 제목 항목**과 **공백만인 공지**에서 화면에만 있는 것이 생겼다 — 그 둘은 저장에서 버려지고
  `isWeekDirty` 도 같은 정규형을 보므로 **dirty 가 아니라 다운로드가 열려 있다.** 즉 "보이는 것 =
  받는 것"이 정확히 거기서 깨진다. 이 저장소는 그 함수를 이미 갖고 있었고 카드만 날것을 받았다.
  **한 자리를 고치면 형제를 전수로 훑는다** — `entries` 를 고치고 `note` 를 빠뜨려 리뷰가 한 번
  더 잡았다. 재현 조건도 좁다: `addEntry` 는 그날 시각이 미정일 때만 기본값을 세우므로 **빈 날에
  더하면 days 가 바뀌어 dirty 가 되어** 구멍이 안 열린다 — 이미 시각이 있는 날에서만 드러난다.
- **`opacity` 는 조상에 걸면 합성 그룹이 되어 자손이 못 되돌린다.** "덩어리는 흐리되 그중 하나만
  또렷" 이 필요하면 조상 opacity 를 **쓸 수 없다**(자식에 `opacity: 1` 을 줘도 안 된다). 게다가
  조상·자손이 곱해져 `.5 × .5 = .25` 가 되므로, 같은 덩어리 안에서 `:disabled` 가 붙은 것과 아닌
  것이 서로 다른 세기로 흐려진다. 흐림은 **한 겹으로만** 걸고 "영역이 잠겼다"는 신호는 테두리·
  배경 톤이 낸다.
- **잠금은 빠져나갈 길을 하나는 남긴다.** 휴방인 날의 조작을 전부 막았더니 "+항목 추가로 빈 줄을
  만든 뒤 그 날을 휴방으로" 흐름이 **저장도 수정도 삭제도 안 되는** 막다른 골목이 됐다 — 빈 제목이
  저장을 막는데(`firstBlankTitleEntry`) 그 오류 문구가 안내하는 두 길("제목을 채우거나 삭제")을
  화면이 둘 다 막고 있었다. **오류 문구가 제시하는 길은 반드시 열려 있어야 한다.**
- **`dialog` 에 `display` 를 무조건 걸면 브라우저 기본 `dialog:not([open]) { display: none }` 을
  이겨 닫힌 창이 화면에 남는다.** "열렸을 때만 마운트한다"는 JS 규약으로 가려질 뿐이라, 나중에
  `open` 속성으로 여닫게 바꾸는 순간 드러난다. `[open]` 으로 스코프한다.
- **한 컴포넌트가 화면에 두 번 뜨면 그 안의 `data-od-id` 가 중복돼 무관한 e2e 가 깨진다.** 확대
  창이 같은 카드를 한 번 더 그리면서 생긴 자리 — 사본 쪽이 이름을 내려놓게 스위치를 둔다
  (`WeekCard` 의 `identified`). 캡처용 `ref` 도 같은 이유로 사본엔 안 건다(두 번째 마운트가
  ref 를 덮고 닫을 때 null 로 만들면 다음 다운로드가 죽는다).
- **요소 스크린샷이 뷰포트보다 크면 sticky 가 그림 중간에 겹쳐 구워진다.** Playwright 가 요소를
  스크롤해 이어 붙이는데 nav·저장 바가 그 자리에 남는다 — 대상 전체가 들어가는 뷰포트로 찍는다.
  그리고 **`?week=` 없이 찍으면 "오늘" 칩이 날짜마다 스냅샷을 흔든다**(간헐 실패를 설계로 없애려면
  주를 고정한다).
- **`e2e:visual:update` 는 전체를 재생성한다 — 무관한 베이스라인까지 덮는다.** 이번에 home·games
  4장이 함께 갱신됐는데, main 에서 돌려 보니 **원래부터 이 머신에서 안 맞던 것**이었다(선행 상태).
  갱신 뒤엔 `git status` 로 의도한 파일만 바뀌었는지 보고, 아니면 되돌린다.
- **`e2e/*.spec.ts` 는 Node 타입 환경에서 검사된다 — `page.evaluate` 안이라도 DOM 전역 이름이
  겹치면 tsc 가 다른 선언을 잡는다.** `document.body.append(el)` 이 "Argument of type
  'HTMLDialogElement' is not assignable to 'string | ReadableStream | Response'" 로 죽었다.
  `appendChild` 는 겹치지 않는다.
- **1Password 앱이 잠기면 `ssh-add -l` 은 성공하는데 서명만 거부된다**("failed to fill whole
  buffer"). 이 저장소는 squash 머지라 main 에 남는 커밋은 **GitHub 이 PGP 로 다시 서명하므로**
  (실측: `committer=noreply@github.com`) 브랜치 커밋 하나를 서명 없이 만들어도 main 의 서명
  상태는 안 바뀐다 — 근거를 확인한 뒤에만 그렇게 하고, 커밋 메시지에 사유를 남긴다.

폼 구조 개편(#56 결정 32·33, PR #139)에서 밟은 것들:

- **조건부로 렌더되던 컨트롤을 늘 렌더되게 옮기면 `.first()` 가 조용히 다른 대상을 가리킨다.**
  휴방 체크박스가 펼친 패널 안에만 있던 시절엔 화면에 하나뿐이라 `.first()` 가 곧 "지금 편집
  중인 날"이었다. 머리 줄로 올리자 일곱 개가 늘 DOM 에 있어 `.first()` 가 **월요일**이 됐고,
  수요일을 쉬게 하려던 시각 스펙이 월요일을 쉬게 해 카드에서 월요일 항목이 사라졌다. 스모크
  147개는 전부 초록이었다 — **시각 스펙만** 그걸 봤고, 그것도 스냅샷 차이가 아니라 그 앞의
  `toContainText` 로 걸렸다. 같은 파일 안에서도 규칙이 갈린다(패널 안에 남은 `add`·`title` 은
  `.first()` 가 그대로 유효하다). 렌더 조건을 넓히는 변경을 할 땐 그 od-id 접두사를 `.first()`
  로 잡는 자리를 전수로 훑는다.
- **`hidden` 속성으로는 "DOM 에 있나"를 재는 테스트의 이빨을 못 증명한다.** 컨트롤을 다시 패널
  안으로 되돌리는 회귀를 흉내 내려고 `hidden={!expanded}` 를 넣었는데 `querySelector` 도
  `fireEvent` 도 그대로 통해 **테스트가 전부 초록이었다** — 치환은 됐고(되읽어 확인했다) 재는
  것이 달랐을 뿐이라, 조용한 no-op 보다 알아채기 어렵다. 언마운트를 재는 단언의 이빨은
  언마운트로만 증명한다(`{expanded && (…)}`).
- **`aria-expanded` 를 가진 요소가 안 바뀌게 하는 방법이 "컨트롤을 치우는 것" 하나뿐은 아니다.**
  결정 28 은 머리 줄 전체를 button 으로 만든 탓에 폼 컨트롤을 전부 패널로 내려야 했고, 그
  근거가 넉 달치 주석으로 남아 "여긴 컨트롤을 못 둔다"로 굳어 있었다. 줄을 div 로 두고 button
  을 그 **안**에 넣으면 같은 불변식이 그대로 서고 컨트롤은 형제로 남는다. 제약을 적은 주석을
  읽을 땐 그것이 **금지**인지 그때의 골격이 낳은 **결과**인지 갈라 본다.

## 접근성 기준 (협상 대상 아님)

구 사이트에서 검증된 기준을 그대로 잇는다.

**`e2e/a11y.spec.ts` 가 axe-core 로 자동 스캔한다**(WCAG 2.1 AA) — 4페이지의 **초기·비로그인
상태** + **게임 상세 다이얼로그**, 각각 1280·320 두 폭. 대비·ARIA·라벨·랜드마크는 이제 그 범위
안에서 기계가 세므로 "어기면 빨개지는 것"이다.

**범위 밖을 알고 써야 한다.** 관리자 전용 모달(편집기·제안함·발행 확인·삭제 확인)과 팬 제안
폼은 스캔에 안 들어간다 — 신원이 갈리는 화면은 세션을 심어야 하고, 그건 `narrow-body.spec.ts`
가 세션을 바꿔 가며 재는 자리다. 그 스펙의 **손으로 열거한 터치 타깃 목록도 그대로 유효하다**
(axe 는 44×44 를 이 저장소가 쓰는 형태로 안 센다). 그리고 axe 는 **셀 수 있는 것만** 센다 —
초점 순서가 말이 되는지, 라벨 문구가 뜻이 통하는지, 장식이 정말 장식인지는 사람 몫이다.
초록이 "접근성 OK"라는 뜻은 아니다.
...
- **값·상태를 적는 칸은 문장이 아니라 표기다.** 정의 목록의 값, 칩, 체크박스 라벨처럼 "무엇인가"
  ...
  값은 표기다. 안내문·힌트·오류는 말을 거는 자리라 **문장**으로 쓰되, 그 문장도 합쇼체다(위 규칙) —
  한때 여기 "그 자리의 다정한 해요체는 그대로 둔다"고 적혀 있었는데 2026-07-26 규칙과 정면으로
  부딪혀 지웠다. 이 문단이 가르는 건 **표기냐 문장이냐**이지 어투가 아니다.
...
- **클라이언트 상태 판정: 둘 이상이 함께 바뀌거나 / 불가능한 조합이 표현되거나 / 비동기가
  끼면 `src/core/*.machine.ts`(XState)로 올린다. 그 밖은 `useState`·`useRef`·
  `useSyncExternalStore` 그대로 둔다**([ADR-0026](./docs/adr/0026-xstate-unifies-client-state.md)).
  폼 값 하나(혼자 바뀌고 조합이 없다), DOM 핸들 `useRef`(직렬화·SSR 불가), `use-theme`(정본이
  이미 `data-theme` 속성이라 머신을 두면 정본이 둘이 된다)는 이 판정으로 걸러진다.

  **왜:** `GameBoard` 하나가 `useState` 12개로 모달 스택 6개를 조립하며 불가능한 조합(수정
  모달과 삭제 확인이 동시에 열린 상태)이 타입으로 표현 가능해졌고, 같은 `error`+`closing`+
