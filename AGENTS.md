...
로그인 상태가 필요한 스펙은 `e2e/session.ts` 의 `signIn(context, baseURL)` 로 세션을 심는다 —
치지직 OAuth 를 태우지 않고 access 쿠키를 직접 서명한다. 마크업 주입이 아니라 진짜 서버
경로(미들웨어 검증 → `getServerActor` → 컴포넌트)가 돈다. 배선의 "왜"는 아래 지뢰 목록에.
...
- **e2e 의 로그인 상태는 `.dev.vars.e2e` + `NEXT_DEV_WRANGLER_ENV=e2e` 로 만든다 —
  process.env 로는 세션 키를 못 넣는다.** wrangler 는 로컬 시크릿을 `.dev.vars.<환경>` →
  `.dev.vars` → `.env`/process.env 순으로 찾는데 **앞이 하나라도 있으면 뒤는 아예 안 본다**.
  대부분의 로컬엔 `.dev.vars` 가 있으니 dev 서버에 테스트 키를 주입할 길은 환경별 파일뿐이다.
  그래서 playwright.config 가 환경명을 `e2e` 로 넘기고, dev 서버보다 **먼저**
  `scripts/e2e-dev-vars.mjs` 가 테스트 전용 EdDSA 키를 `.dev.vars.e2e` 에 심는다(globalSetup
  은 늦다 — Playwright 는 webServer 를 globalSetup 보다 먼저 띄운다). `wrangler.jsonc` 에
  `env.e2e` 섹션은 **만들지 않는다**: 만들면 `d1_databases` 같은 비상속 키가 안 딸려와 DB
  바인딩이 사라진다. 섹션이 없을 때 나오는 "No environment found" 경고 두 줄이 정상 상태다.
  테스트는 그 키로 access 쿠키를 직접 서명해 심는다(`e2e/session.ts`) — 서버는 진짜 세션과
  구별하지 못하고, 쿠키는 `__Host-` 라 `https://localhost` url 로 심어야 CDP 가 받는다.
...
- 사용자 노출 문구는 전부 한국어. 톤은 다정하고 장난기 있되 과하지 않게. 고양이 말투("냐")는
  화면당 1회까지.
- **형식적·파괴적 화면은 예외로 합쇼체다.** 삭제 확인처럼 되돌릴 수 없는 행동을 묻는 자리에서
  해요체·요체는 정이 떨어지고 조잡하게 읽힌다 — "보드에서 뗄까요? / 떼면 되돌릴 수 없어요"가
  아니라 "삭제하시겠습니까? / 삭제하면 되돌릴 수 없습니다"다. 격식이 "이건 진짜다"를 말하는
  자리이고, 장난기는 여기서 신뢰를 깎는다. 나머지 화면의 다정한 톤은 그대로다.
