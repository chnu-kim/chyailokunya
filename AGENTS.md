...
**`e2e/a11y.spec.ts` 가 axe-core 로 4페이지 × 2폭(1280·320)을 자동 스캔한다**(WCAG 2.1 AA).
대비·ARIA·라벨·랜드마크는 이제 기계가 세므로, 이 목록에서 그 셋은 "지켜야 할 것"이자 "어기면
빨개지는 것"이다. **다만 axe 는 셀 수 있는 것만 센다** — 초점 순서가 말이 되는지, 라벨 문구가
뜻이 통하는지, 장식이 정말 장식인지는 여전히 사람 몫이다. 초록이 "접근성 OK"라는 뜻은 아니다.
그리고 `narrow-body.spec.ts` 의 **손으로 열거한 터치 타깃 목록은 그대로 유효하다** — axe 는
44×44 규칙을 이 저장소가 쓰는 형태로 세지 않는다.
