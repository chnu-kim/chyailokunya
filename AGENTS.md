...
(`--project=visual`)는 로컬 dev 회귀 + 사람의 육안 패리티 판단용이다. 배포는 CI 가 아니라
**Cloudflare Workers Builds** 가 main 갱신 시 맡는다([ADR-0009](./docs/adr/0009-actions-gate-workers-builds.md)).
...
- **localStorage 는 외부 스토어다 — 마운트 로드를 effect+setState 로 하면 위 set-state-in-effect
  지뢰에 걸린다.** 게임 보드는 `src/app/games/games-store.ts`(모듈 싱글턴 + 리스너)를
  `useSyncExternalStore` 로 읽는다: getServerSnapshot=시드로 SSR·수화하고 getSnapshot=localStorage
  로 마운트 후 갱신 → 수화 불일치 없이 저장 목록을 반영한다(테마 토글과 같은 패턴).
