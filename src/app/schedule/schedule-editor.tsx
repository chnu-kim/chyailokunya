...
/* 발행 체크 기본값. 세 경우를 가른다:
   - 이미 발행된 주(publishedAt 있음) → 켜짐.
   - **이관된 레거시 주**(메타 없음 + 항목 있음) → 켜짐. 그 항목들은 이미 보드에 날짜로 떠
     있는데(ADR-0022 레거시 규칙: 메타 부재 = 표시), 발행을 끈 채 저장하면 published_at NULL 인
     메타가 생겨 그 주의 과거 플레이 날짜가 보드에서 사라진다(손실 0 이 첫 편집에서 깨진다).
     "이미 공개 중"으로 열어 그대로 저장하면 발행 시각이 찍혀 계속 보인다.
   - **브랜드-뉴 주**(메타 없음 + 항목 없음) → **꺼짐**. 관리자가 다음 주를 처음 짜는 자리라,
     발행을 미리 켜 두면 결정 13(초안 우선, 미완성이 og 카드로 안 나감)에 어긋난다 — 짜는
     중엔 초안이 기본이고 발행은 다 되면 관리자가 켠다. 레거시와 달리 지킬 과거 날짜가 없다.
   즉 메타 없는 주의 기본값을 "항목이 있느냐"로 가른다: 있으면 레거시(지켜야 함)라 켜고,
   없으면 새 주라 끈다. */
function weekToDraft(week: WeekView): WeekDraft {
  return {
    note: week.note ?? "",
    published: week.publishedAt !== null || (!week.hasMeta && week.entries.length > 0),
