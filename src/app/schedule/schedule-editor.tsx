    ...
    <section className="sched sched--edit" data-od-id="schedule-editor">
      <div className="wrap">
        ...
        {/* ── 2열 골격(결정 24) · 메타 재배치(결정 29, 2026-07-31 개정) ────────────────
            ...
            팬아트·작가표기는 미리보기 카드에만 그려지는 값이라, 결정 24 초판처럼 요일 **위**
            에서 226px 를 먹는 자리(`메타 → 미리보기 → 요일`)는 그 값이 어디로 가는지 보여주는
            미리보기와 반대편이라 어색했다 — 결정 29 가 미리보기 아래로 옮겼다. (그때는 공지
            입력이 함께 있어 카드의 좌우 배치까지 흉내 냈는데, 공지가 걷히며 그 대칭은
            없어졌다 — 아래 팬아트 상자 주석.)
            ...
            **narrow flow(1440 미만) 순서는 결정 29 코멘트가 제안한 `요일 → 미리보기 → 메타`가
            ...
            1440 이상에서만 `grid-template-areas` 로 요일을 왼쪽 열에, 곁칸을 오른쪽 열에
            ...
            **알고 받아들인 트레이드오프 — 1440 이상에서 탭 순서가 시각 순서(왼쪽 요일 → 오른쪽
            곁칸)와 다르다(곁칸 → 요일).** 적대적 리뷰가 이 자리를 세 라운드 연속으로 지적했다
            (2026-07-31~08-01). DOM 순서 하나가 두 폭 모두를 섬겨야 하는데 라운드마다 요구가
            서로 배타적이다 — narrow flow 회귀(위 문단)는 "곁칸을 요일보다 앞에" 를 요구하고,
            탭 순서 지적은 정확히 그 반대("요일을 곁칸보다 앞에")를 요구한다. `order` 로 시각만
            돌리면 tab 순서와 시각 순서가 그 안에서 다시 갈라져(결정 24 가 애초에 금지한 바로
            그 증상) 문제를 옮길 뿐 없애지 못하고, 폭별로 다른 DOM을 내보내는 건 이 개편의
            범위를 넘는 과설계다. main 의 원래 순서(`메타 → 미리보기 → 요일`)도 같은 부류의
            불일치였다 — 탭이 왼쪽 위(메타)에서 시작해 오른쪽(미리보기)으로 갔다가 다시 왼쪽
            아래(요일)로 돌아갔다. 이 PR 은 그 불일치를 "탭이 오른쪽 위에서 시작"으로
            바꿨을 뿐 새로 만들지 않았다 — 다만 main 은 왼쪽 위에서 시작해 관례에 더
            가까웠으므로 아주 없던 일은 아니다. 그 미세한 후퇴보다 narrow flow 회귀(미리보기가
            화면 밖으로 밀림)와 결정 29 의 시각 요구(메타가 카드 아래)가 더 크다고 판단해
            이 순서를 그대로 둔다.

            **3라운드 지적("미리보기는 narrow 전용으로 앞에 두고 메타의 인터랙티브 컨트롤만
            뒤로 미루라")은 전제가 틀렸다** — 미리보기(`WeekCardDownload`) 자신이 이미 인터랙티브
            버튼 둘("원본 크기로 보기"·"PNG 다운로드")을 갖고 있다(week-card-download.tsx). 그
            제안대로 갈라도 미리보기의 그 버튼들이 여전히 요일보다 먼저 탭돼 지적이 그대로
            재발한다 — 컴포넌트를 직접 확인해 반박한다(AGENTS "적대적 리뷰가 라운드마다 축을
            바꾸면 근거 대고 기각하는 지점이 있다"). */}
        <div className="sched__grid">
          <div className="sched__aside">
            {/* **`sched__aside-pin` 이 왜 또 한 겹인가** — sticky 를 이 안쪽 겹에 걸어야
                한다(2026-07-31, 적대적 리뷰가 잡은 회귀의 두 번째 원인). 처음엔 sticky 를
                `.sched__aside`(이 바깥 겹, grid-area 를 가진 그 자체) 에 바로 걸었는데, 그러면
                sticky 인 요소와 `align-items: stretch` 로 늘어나는 요소가 같아져 버려 늘어난
                높이가 곧 sticky 의 포함 블록 높이와 같아진다 — 포함 블록보다 큰(또는 같은)
                요소는 그 안에서 이동할 여유(slack)가 0이라 sticky 가 사실상 안 걸린다(실측:
                스크롤해도 그냥 문서를 따라 그대로 밀려 올라간다, 100% 되돌려짐). 안쪽에 한
                겹(`sched__aside-pin`)을 더 두면 그 안쪽 겹의 자연 높이(미리보기+메타, ~700px대)가
                바깥 겹의 늘어난 높이(요일 목록과 같은 높이, 900px대일 수 있다)보다 작아 그
                차이만큼 slack 이 생기고, sticky 가 그 여유 안에서 정상 작동한다 — 이게 결정 24
                가 세운 "작은 sticky 요소를 큰 포함 블록 안에" 패턴이고, 이 겹이 없으면 그 패턴이
                깨진다. */}
            <div className="sched__aside-pin">
              {/* 넓은 폭에선 오른쪽 열 위 칸에 sticky 로 붙고, 1440 미만에선 요일 목록보다 **앞**에
                  그대로 흐른다(위 sched__grid 주석 참고) — 어떤 날을 펼쳐도(결정 28) 이 위치가
                  안 바뀐다. 미리보기·메타가 이 안쪽 겹에서 함께 sticky 로 붙어야(아래 sched__grid
                  주석의 겹침 회귀 참고) 메타가 스크롤 중 미리보기 밑으로 들어가지 않는다. */}
              <div className="sched__preview">
                <WeekCardDownload
                  card={card}
                  weekStartDate={weekStartDate}
                  /* 관리자 화면이라 다운로드가 아이콘이다(결정 35) — 팬이 보는 읽기 화면은
                     글자를 남긴다(그 근거는 week-card-download.tsx 의 variant 주석). */
                  variant="editor"
              ...
              {/* ── 주 메타: 팬아트(결정 29 · 공지 입력 제거 2026-08-01) ──────────────────
                미리보기 카드에만 그려지는 값이라 그 카드 바로 다음에 선다.

                **한때 여기 공지 입력이 함께 있었고 두 박스가 카드의 좌우 배치를 흉내 냈다**
                (카드에서 공지는 하단 왼쪽 글, 그림은 오른쪽 — week-card.tsx 의
            ...
            **입력 컨트롤이 있으므로 사진지 섬(.polaroid)을 쓰지 않는다**: 그 섬은 다크에서
            ...
            씻긴다(kunya-design §4). 대신 .paper(--surface)로 상자 테두리만 준다 — 안의
            .sched-field 는 --border-strong 이라 같은 표면 위에서도 대비가 선다(.sched-day
            카드가 이미 같은 조합으로 검증돼 있다). 읽기 화면은 조작이 없어 그쪽이 폴라로이드다
            — 같은 값을 두 화면이 다른 부품으로 그리는 게 맞는 자리다.
          ...
          <ol className="sched__days" data-od-id="schedule-days">
            {days.map((date, i) => {
              ...
              /* 휴방 표시는 **줄 전체가 진다**(결정 32). 전엔 접힌 줄에 `chip--warn 휴방` 칩이
                 서서 "이번 주 어느 날이 쉬나"를 한눈에 말했는데, 칩 자리가 조작 가능한 체크박스로
                 바뀌며 그 큰 신호가 체크 표시 하나로 줄었다 — 그 자리를 카드 톤이 대신 받는다
                 (`.sched-day--rest`, schedule.css). 칩을 체크박스 옆에 함께 두는 길은 안 택했다:
                 같은 사실을 한 줄에서 두 번 말하게 되고, 그게 정확히 이 개편이 없애는 중복이다. */
              return (
                <li
                  key={date}
                  className={[
                ...
                >
                  {/* ── 하루 머리 줄(결정 32, 2026-08-01) ──────────────────────────────────
                      **접힌 채로 시각·휴방을 조작한다.** 전엔 이 줄이 `19:00` 을 보여주기만 하고
                      펼치면 같은 값이 별도 줄의 입력으로 다시 나타나 접힘↔펼침이 다른 언어였다 —
                      "이번 주는 수·목만 휴방" 같은 흔한 작업이 펼치기 7번을 요구했다. 펼침은 이제
                      **항목 편집 전용**이다.
                  ...
                  <div className="sched-day__head">
                    ...
                    {/* 하루의 속성(이슈 #117) — 시각과 휴방. 항목이 아니라 하루에 딸린 값이라
                        ...
                        날에 시작 시각을 붙이는 건 뜻이 안 맞고, 잠가 두면 그 조합을 만들 길
                        자체가 없어진다(표시에서 휴방이 이긴다는 결정 5 를 입력 단계에서 미리
                        지킨다). 둘을 한 겹으로 묶는 이유는 560 이하에서 **둘이 함께** 다음 줄로
                        내려가야 해서다 — 감싸지 않으면 flex 가 base size 로 줄바꿈을 계산해
                        시각만 첫 줄에 남는다(schedule.css 의 감축 경로 주석). */}
                    <div className="sched-day__when">
                      <label className="sr-only" htmlFor={`${date}-time`}>
                        {formatMD(date)} 방송 시작 시각
                      </label>
                      <input
                        id={`${date}-time`}
                        className="sched-field sched-day__time-field"
                        type="time"
                        value={day.startTime}
                        disabled={day.rest}
                      ...
                      />
                      {/* **날짜를 접근 가능한 이름 안에 넣는다**(결정 32). 전엔 이 체크박스가
                          펼친 패널 안에 있어 한 번에 하나만 존재했고, "휴방" 하나로도 어느
                          하루인지가 문맥으로 정해졌다. 이제 일곱 개가 늘 탭 순서에 있으므로
                          이름이 전부 "휴방"이면 스크린 리더에선 구별이 안 간다 — 시각 입력이
                          이미 `sr-only` 라벨로 날짜를 짚고 있던 것과 같은 처방이다.
                          `aria-label` 이 아니라 보이는 글자 **앞의** sr-only 조각인 이유:
                          이름이 "4.1 휴방"으로 조립돼 보이는 글자를 그대로 품는다(WCAG 2.5.3
                          label-in-name — `aria-label` 로 갈아 끼우면 그 포함 관계를 손으로
                          유지해야 한다). */}
                        ...
                        {dayEntries.map((e) => {
                         ...
                         그래서 접근 가능한 이름과 툴팁이 게임명을 그대로 말한다. */
                          ...
                          return (
                            ...
                            >
                              {/* ── 항목 한 줄 = 한 상자(결정 33, 2026-08-01) ────────────────
                                전엔 표지·제목·삭제가 각자 테두리를 갖고 하루 카드 > 패널 >
                                필드로 같은 무게의 테두리가 세 겹 중첩됐다(사용자가 "흩뿌려진
                                느낌"이라 부른 자리 중 하나). 이제 **상자는 이 행 하나**이고 안쪽
                                컨트롤은 테두리를 안 그린다 — 중첩이 두 겹으로 준다.
