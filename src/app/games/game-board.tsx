...
export function GameBoard({
  ...
  function onRemoved(row: GameRow) {
              ...
              return (
                ...
                >
                  ...
                  {/* 수정·삭제는 썸네일 우상단에 겹쳐 뜬다. 카드 본문에 라벨 줄로 있던 걸 옮긴
                      ...
                      라벨은 sr-only 로 내려가고 그 자리에 동사를 넣는다: 아이콘만 남으면
                      접근 이름이 게임명뿐인 정체불명 버튼 둘이 된다. 서버가 인가를 다시
                      검사하므로 이 분기는 편의일 뿐이다(불변식 3). */}
                  {(canWrite || canDelete) && (
                    <div className="game__acts">
                      {canWrite && (
                        ...
                        >
                          <span className="sr-only">{g.categoryValue} 날짜 수정</span>
                        </button>
                      )}
                      {canDelete && (
                        <button
