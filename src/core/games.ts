...
/* ── 플레이 날짜를 게임 폼에서 고칠 수 있는가 ────────────────────────────────
   정본은 일정(schedule_entries)이고 게임 폼은 **또 하나의 입구**다(이슈 #56 결정 3 유지).
   입구가 다뤄도 되는 건 그 게임에 걸린 항목이 0개(새로 만든다) 또는 1개(그 항목의 날짜를
   옮긴다)일 때뿐이다.

   여러 날 편성("월·화 젤다")을 막는 이유는 입력 하나로 표현이 안 되기 때문이다. 억지로 하나만
   고치면 나머지 날은 **화면에 안 보인 채** 남아, 관리자가 "7/22 로 고쳤다"고 믿는데 보드는
   여전히 MAX(7/23)를 그린다 — 고쳤는데 안 바뀐 것처럼 보이는 자리다. 그래서 그 경우는
   /schedule 편집기로 보낸다(거기선 7일이 한눈에 보이고 항목마다 시각·제목이 있다).

   서버 검증과 UI 잠금이 이 함수 하나를 나눠 쓴다 — 판정이 갈리면 폼은 잠갔는데 서버는
   통과시키는(또는 그 반대) 상태가 생긴다. */
export function isPlayDateEditable(dates: readonly string[]): boolean {
