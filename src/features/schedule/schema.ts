...
/* 그림의 픽셀 치수 — 읽기 화면이 `<img width height>` 로 자리를 예약하는 데만 쓴다
   (db/schema.ts 가 왜 컬럼으로 두는지의 정본). **관리자 브라우저가 파일에서 읽어 보내는
   클라이언트 주장이다** — 바이트와 형식은 업로드가 매직 바이트로 판정하므로 여기서 틀릴 수 있는
   것은 레이아웃뿐이고, 그래서 신뢰가 아니라 **상한**만 건다. 20000 은 현실적인 이미지 치수의
   위쪽이고, 상한이 없으면 거대한 정수가 그대로 img 속성에 실려 브라우저가 그만큼 빈 자리를
   비운다(그림 한 장이 화면을 통째로 밀어낸다).

   상한 값은 core/fanart 가 정본이다 — 치수를 읽어 보내는 화면이 같은 값을 봐야 한다(갈리면
   화면이 읽은 값이 여기서 거절돼 그림 자체를 못 건다, 그 파일의 주석 참고). */
