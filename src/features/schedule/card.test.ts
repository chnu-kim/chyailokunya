...
describe("buildWeekCard", () => {
  it("주 범위 라벨과 요일 라벨을 weekDates 순서 그대로 접는다", () => {
    const card = buildWeekCard(MON, week());
  ...
  it("공지는 그대로 넘긴다(null 도 그대로)", () => {
    expect(buildWeekCard(MON, week({ note: "공지" })).note).toBe("공지");
    expect(buildWeekCard(MON, week({ note: null })).note).toBeNull();
  });
  ...
  it("하루 여러 항목 — 정렬은 입력 순서를 그대로 믿는다(SQL 이 이미 정렬)", () => {
    ...
    const card = buildWeekCard(MON, week({ entries }));
